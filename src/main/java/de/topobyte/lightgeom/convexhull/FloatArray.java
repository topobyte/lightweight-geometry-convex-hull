 ...
 * with {@code ArrayList<Float>}. If unordered, this class avoids a memory copy
 * when removing elements (the last element is moved to the removed element's
 * position).
 * 
 * @author Nathan Sweet
 */
public class FloatArray
{
	...
	public void swap(int first, int second)
	{
		if (first >= size) {
			throw new IndexOutOfBoundsException(
					"first can't be >= size: " + first + " >= " + size);
		}
		if (second >= size) {
			throw new IndexOutOfBoundsException(
					"second can't be >= size: " + second + " >= " + size);
		}
		float[] items = this.items;
	...
	public void removeRange(int start, int end)
	{
		...
		if (ordered) {
			System.arraycopy(items, start + count, items, start,
					size - (start + count));
		} else {
