...
// This file has been imported from the libGDX project and has been modified
// thereafter. It follows the original Copyright notice and license header:
...
public class ConvexHull
{
	 ...
	 * {@link #computePolygon(float[], int, int, boolean)} but returns indices
	 * of the specified points.
	 */
	public IntArray computeIndices(float[] points, int offset, int count,
			boolean sorted, boolean yDown)
	{
		int end = offset + count;
		...
		// Upper hull.
		for (int i = end - 4, index = i / 2, t = hull.size
				+ 2; i >= offset; i -= 2, index--) {
			float x = points[i];
			float y = points[i + 1];
			while (hull.size >= t && ccw(x, y) <= 0) {
				hull.size -= 2;
	...
	private void sortWithIndices(float[] values, int count, boolean yDown)
	{
		...
		while (stack.size > 0) {
			...
			int i = quicksortPartitionWithIndices(values, lower, upper, yDown,
					originalIndicesArray);
			if (i - lower > upper - i) {
				stack.add(lower);
				stack.add(i - 2);
			}
	...
	private int quicksortPartitionWithIndices(final float[] values, int lower,
			int upper, boolean yDown, short[] originalIndices)
	{
		float x = values[lower];
		float y = values[lower + 1];
		int up = upper;
